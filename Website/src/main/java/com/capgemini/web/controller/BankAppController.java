...
public class BankAppController {
	...
	public String withdraw(@ModelAttribute Transaction transaction, Model model) {
		restTemplate.postForEntity("http://TransactionService/transactions/withdraw", transaction, null);
	...
	public String deposit(@ModelAttribute Transaction transaction, Model model) {
		restTemplate.postForEntity("http://TransactionService/transactions", transaction, null);
		model.addAttribute("message", "Success!");
		return "DepositForm";
	}
	...
	public String fundTransfer(@RequestParam("senderAccountNo") int senderAccountNo,
		...
		restTemplate.postForEntity("http://TransactionService/transactions/withdraw", senderTransaction, null);
		Transaction receiverTransaction = new Transaction(receiverAccountNo, amount);
		restTemplate.postForEntity("http://TransactionService/transactions", receiverTransaction, null);
		model.addAttribute("message", "Success!");
		return "fundTransfer";
	}
	...
	public ModelAndView getStatement(@RequestParam("offset") int offset, @RequestParam("size") int size) {
		...
		CurrentDataSet currentDataSet = restTemplate.getForObject("http://TransactionService/transactions",
